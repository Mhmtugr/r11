...
class ErpService {
  ...
  async setupWebSocketConnection() {
    try {
      if (apiService.erpMockMode) { // ERP'ye özel mock modunu kullan
        logger.warn('ERP Servisi için Mock modda WebSocket bağlantısı kurulmayacak');
        syncStatus.value = 'connected'; // Durumu bağlı olarak ayarla
        return;
      }
      ...
      const token = await this.authStore.getToken(); // Changed to await
      if (!token) {
  ...
  async getProductionData() {
    ...
    try {
      const response = await apiService.get('/erp/production-data');
  ...
  // Örnek Mock Fonksiyonları (Gerçek implementasyonunuza göre detaylandırın)
  getMockStockData(productCode) {
    return new Promise(resolve => {
      setTimeout(() => {
        resolve({ productCode, stockLevel: Math.floor(Math.random() * 100), unit: 'adet' });
      }, 500);
    });
  }

  getMockMaterialData(materialId) {
    return new Promise(resolve => {
      setTimeout(() => {
        resolve({ materialId, name: `Mock Malzeme ${materialId}`, description: 'Bu bir mock malzeme açıklamasıdır.', stock: Math.floor(Math.random() * 50) });
      }, 500);
    });
  }

  mockReserveMaterial(orderId, materialId, quantity) {
    return new Promise(resolve => {
      setTimeout(() => {
        resolve({ success: true, reservationId: `MOCK_RES_${Date.now()}`, message: `Malzeme ${materialId} (${quantity} adet) sipariş ${orderId} için mock olarak rezerve edildi.` });
      }, 500);
    });
  }

  getMockMaterialAvailability(materialId) {
    return new Promise(resolve => {
      setTimeout(() => {
        const isAvailable = Math.random() > 0.2; // %80 ihtimalle mevcut
        resolve({ materialId, available: isAvailable, quantity: isAvailable ? Math.floor(Math.random() * 100) : 0 });
      }, 500);
    });
  }
}
