      ...
      <div class="ai-chat-content" ref="chatContent">
        ...
        <div v-for="(message, index) in history" :key="index" 
          ...
          <div class="ai-message-content">
            ...
            <!-- PROAKTİF ANALİZ/ÖNERİ KARTI -->
            <ProactiveAnalysisCard
              v-else-if="message.intent === 'proactive_analysis'"
              :intent="message.intent"
              :data="message.data"
              :actions="message.data && message.data.actions ? message.data.actions : []"
...
// Servisler
// const aiServiceComposable = useAiService() // REMOVED: This was causing the error.
const technicalStore = useTechnicalStore();
const route = useRoute();

// Destructure properties from the imported aiService object.
// Note: Some of these (supportedModels, switchModel, etc.) are not yet on aiService.
// They will be added in the next step by modifying ai-service.js.
const { 
  sendMessage, 
  history, 
  isProcessing, 
  ...
  modelComponents,
  modelMeasurements
  // Properties like setActiveAiService, getCurrentConfig are available on aiService
  // but AIChatModal expects switchModel and getCurrentModel with specific behavior.
} = aiService; // Use the imported aiService object directly.
...
const handleSendMessage = async () => {
  ...
  // MODIFICATION START: Add console logs for debugging
  defaultLogger.debug('[AIChatModal] handleSendMessage called.');
  defaultLogger.debug(`[AIChatModal] userInput: ${userInput.value}`);
  defaultLogger.debug(`[AIChatModal] isProcessing: ${isProcessing.value}`);
  // MODIFICATION END

  if (!userInput || !userInput.value || !userInput.value.trim() || (isProcessing && isProcessing.value)) {
    defaultLogger.warn('[AIChatModal] handleSendMessage: Aborting due to empty input or processing state.');
    return;
  }
  ...
  nextTick(() => {
    const inputElement = document.querySelector('.ai-chat-input input[type="text"]') // Daha spesifik bir seçici
    if (inputElement) {
      inputElement.focus()
    }
  });
  ...
  try {
    ...
    defaultLogger.info(`[AIChatModal] handleSendMessage - Current context:`, currentContext);
    defaultLogger.debug(`[AIChatModal] Calling aiService.sendMessage with text: "${userMessageText}"`); // MODIFICATION: Log before calling service

    // Standart mesaj gönderimi, artık aiService.sendMessage'i doğrudan kullanmıyoruz,
    // bunun yerine props'tan gelen sendMessage'i kullanıyoruz.
    // Ancak, aiService'den gelen sendMessage'i kullanmak daha mantıklı görünüyor.
    // Bu kısmı tekrar gözden geçirelim. Şimdilik aiService'den gelen sendMessage'i kullanalım.
    let response = await aiService.sendMessage(
      { text: userMessageText }, // Sadece metin gönderiyoruz, resim vb. eklenebilir
      [], // conversationHistoryIgnored - aiService kendi geçmişini yönetiyor
      currentContext // Yakalanan bağlamı iletiyoruz
    );
    
    defaultLogger.debug('[AIChatModal] aiService.sendMessage response:', response); // MODIFICATION: Log response

    // Teknik dokümanlarda ara
    if (shouldSearchTechnical.value) {
      await searchTechnicalDocuments(userMessageText)
    }
    
    // CAD modelleri ara
    if (shouldSearchCADModels.value && !response?.modelPreview) {
      await processCADModelRequest(userMessageText)
    }
...
// --- Proaktif analiz mesajlarını otomatik ayrıştıran watcher ---
watch(history, (newHistory) => {
  if (!Array.isArray(newHistory)) return;
  newHistory.forEach((msg, idx) => {
    if (msg && msg.intent === 'proactive_analysis' && !msg.data && typeof msg.content === 'string') {
      try {
        // JSON string ise ayrıştır
        if (msg.content.trim().startsWith('{') || msg.content.trim().startsWith('[')) {
          const parsed = JSON.parse(msg.content);
          history.value[idx].data = parsed;
        }
      } catch (e) {
        // Ayrıştırılamazsa kullanıcıya uyarı ekle
        history.value[idx].data = { error: 'Proaktif analiz verisi ayrıştırılamadı.' };
      }
    }
  });
}, { deep: true })
...
onMounted(() => {
  ...
  nextTick(() => {
    const inputElement = document.querySelector('.ai-chat-input input[type="text"]');
    if (inputElement) {
      inputElement.focus();
    }
  });
...
function handleProactiveAction(action) {
  // Burada aksiyona göre işlem yapılabilir (ör: analiz, yönlendirme, modal açma vs.)
  // Örnek:
  if (action === 'analiz' || action === 'Analiz Et') {
    // Analiz modalı açılabilir
    // showAnalysisModal = true;
  } else if (action === 'stok detay' || action === 'Stok Detayı') {
    // Stok detayına yönlendirme
  }
  // ... diğer aksiyonlar ...
}
...
function openOrderDetail(orderNo) {
  ...
  // örn: this.$router.push(`/orders/${orderNo}`)
}

function openStockDetail(stockName) {
  // Stok detay modalı veya yönlendirme
  // örn: this.$router.push(`/inventory/${stockName}`)
}
</script>
