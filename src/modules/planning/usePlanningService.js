...
export function usePlanningService() {
  ...
  const getPlanningData = async () => {
    try {
      ...
      // Schedule oluştur
      const tasks = await generateSchedule(orders);
      
      // Kapasite yükünü hesapla
      const capacityLoad = calculateCapacityLoad(tasks);
      
      // Teslimat tahminlerini oluştur
      const deliveryEstimates = generateDeliveryEstimates(tasks);
  ...
  const generateSchedule = async (orders) => {
    ...
    for (const order of orders) {
      ...
      const unitName = demoProductionUnits.find(u => u.id === resourceId)?.name || 'Genel Montaj';
      ...
      schedule.push({
        ...
        progress: progress,
        status: status,
        description: `${order.quantity} adet ${order.cellType} için üretim görevi. Müşteri: ${order.customerName}`,
  ...
  const calculateCapacityLoad = (schedule) => {
    ...
    schedule.forEach(task => {
      ...
      if (new Date(task.start) < nextWeek && new Date(task.end) > now) {
        const resource = task.resourceId || 'genel_montaj';
        const durationHours = (new Date(task.end) - new Date(task.start)) / (1000 * 60 * 60);
