<template>
  <div class="production-container p-4">
    <AIInsightsDashboard class="mb-4" ref="insightsDashboard" category="üretim" @insights-loaded="onInsightsLoaded" />
    ...
    <!-- Örnek: <UpdatePlanModal v-model:visible="isUpdatePlanModalVisible" @plan-updated="onPlanUpdated" /> -->
    <!-- Örnek: <AddReportModal v-model:visible="isAddReportModalVisible" @report-added="onReportAdded" /> -->
...
import AIInsightsDashboard from '@/components/ai/AIInsightsDashboard.vue'; // Yeni eklenen import
import { updateProductionLearningModel, getProductionLearningSummary } from '@/services/ai-service.js';
...
// Plan güncelleme veya rapor ekleme işlemi sonrası model güncelle
const onPlanUpdated = async (newPlanData) => {
  // ...plan güncelleme işlemleri...
  await updateProductionLearningModel({
    ...newPlanData,
    completedAt: newPlanData.endDate,
    plannedAt: newPlanData.startDate,
    // Diğer gerekli alanlar
  });
  // Güncel özet alınabilir
  const summary = getProductionLearningSummary();
  console.log('Güncellenmiş üretim öğrenme özeti:', summary);
  refreshData();
};

const onReportAdded = async (newReportData) => {
  // ...rapor ekleme işlemleri...
  await updateProductionLearningModel({
    ...newReportData,
    completedAt: newReportData.date,
    // Diğer gerekli alanlar
  });
  const summary = getProductionLearningSummary();
  console.log('Güncellenmiş üretim öğrenme özeti:', summary);
...
function onInsightsLoaded(insights) {
  ...
  if (critical) {
    proactiveCard.value = {
      ...
      action: critical.actions && critical.actions.length ? { label: 'Üretim Detayını Aç', handler: () => alert('Üretim detayına gidiliyor!') } : null
    };
  } else {
