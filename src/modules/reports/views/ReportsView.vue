<template>
  <div class="reports-view">
    <AIInsightsDashboard class="mb-4" ref="insightsDashboard" category="analiz" @insights-loaded="onInsightsLoaded" />
              ...
              <div class="col-12">
                <div class="card bg-light border-0">
                  <div class="card-body text-center">
                    <h5 class="card-title">Detaylı Raporlar</h5>
                    <p class="card-text">Sipariş ve üretim verilerini daha ayrıntılı incelemek için aşağıdaki rapor sayfalarını ziyaret edebilirsiniz.</p>
                    <router-link to="/reports/orders" class="btn btn-info me-2">
                      <i class="bi bi-journal-text me-1"></i> Sipariş Raporları
                    </router-link>
                    <router-link to="/reports/production" class="btn btn-warning">
                      <i class="bi bi-graph-up-arrow me-1"></i> Üretim Raporları
                    </router-link>
                  </div>
                </div>
              </div>
...
function onInsightsLoaded(insights) {
  ...
  if (critical) {
    proactiveCard.value = {
      title: 'Kritik Analiz Uyarısı',
      description: critical.description,
      action: critical.actions && critical.actions.length ? { label: 'Analiz Detayını Aç', handler: () => alert('Analiz detayına gidiliyor!') } : null
    };
  } else {
