...
import { getAuth } from 'firebase/auth'; // Firebase Auth importu eklendi
...
export const useAuthStore = defineStore('auth', () => {
  ...
  async function getToken() {
    const firebaseAuth = getAuth(); // Firebase auth instance'ını al
    const currentFirebaseUser = firebaseAuth.currentUser; // Mevcut Firebase kullanıcısını al

    if (currentFirebaseUser && typeof currentFirebaseUser.getIdToken === 'function') {
      try {
        // Token'ı yenilemeye zorla (true parametresi ile)
        const token = await currentFirebaseUser.getIdToken(true);
        return token;
      } catch (error) {
        console.error("[AuthStore] Firebase Auth üzerinden ID token alınırken hata:", error);
        return null;
      }
    }
    console.warn("[AuthStore] Firebase kullanıcısı mevcut değil veya getIdToken bir fonksiyon değil. Kullanıcı henüz Firebase ile doğrulanmamış veya kimlik doğrulama durumu yükleniyor olabilir.");
    return null;
  }
  ...
  return {
    ...
    logout,
    demoLogin,
    getToken // Export the updated getToken function
  };
});